...
using Beamable.Web3SolanaFederation.Features.Accounts;
...
public class AuthenticateEndpoint : IEndpoint
{
    ...
    public AuthenticateEndpoint(Configuration configuration, RequestContext requestContext, AccountsService accountsService, AuthenticationService authenticationService)
    {
    ...
    public async Promise<FederatedAuthenticationResponse> Authenticate(string token, string challenge, string solution)
    {
        ...
        {
            var microserviceInfo = MicroserviceMetadataExtensions.GetMetadata<Web3SolanaFederation, Web3SolanaIdentity>();
        ...
        if (!string.IsNullOrEmpty(challenge) && !string.IsNullOrEmpty(solution))
        {
            ...
            BeamableLogger.LogWarning("Invalid signature {signature} for challenge {challenge} and account {account}", solution, challenge, token);
